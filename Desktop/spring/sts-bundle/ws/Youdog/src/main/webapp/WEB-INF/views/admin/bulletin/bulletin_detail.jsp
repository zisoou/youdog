...
<div class="container py-5">
	<header class="pb-3 pt-3 mb-4 border-bottom bg-white-- rounded-3">
		<div class="row ">
			<div class="col-4 ">
				<p class="text-left px-4" style="margin-bottom: 0;">
					${bulletinVo.bulletin_writer}</p>
			</div>
			<div class="col-4">
			...
			<div class="col-4">
				<p class="text-right px-4"
					style="text-align: right; margin-bottom: 0;">조회 :
					${bulletinVo.bulletin_views} / 추천 : ${bulletinVo.bulletin_likes}</p>
			</div>
		</div>
	</header>
	...
	<div class="p-5  bg-light rounded-3">
		<div class="row ">
			...
			<div class="col-6">
				<c:if test="${loginedAdminMemberVo.getAdmin_approval() == '2'}">
					<div class="text-right px-2" style="text-align:right; margin-bottom:0;">
		...
		<div class="container-fluid py-5">
			...
			<p class="col-md-8 fs-4 mb-5 pb-5">작성일
				${bulletinVo.bulletin_reg_date}</p>
			<div>
				<p class="fs-5 mb-5">${bulletinVo.bulletin_content}</p>
				...
				<div class="row mt-5">
					...
					<div class="col-4">
						...
						<button class="btn btn-primary-- btn-lg m-auto text-center mx-2"
							type="button">
							추천하기 <i class="fas fa-thumbs-up"></i>
							${bulletinVo.bulletin_likes}
						</button>
						<button class="btn btn-primary-- btn-lg m-auto text-center mx-2"
							type="button">
							혼인신고 <i class="fas fa-thumbs-up"></i>
							${bulletinVo.bulletin_likes}
						</button>
				...
				<div class="row">
					...
					<div class="col-6">
						<c:set var="loginedAdminMemberVo"
							value="${sessionScope.loginedAdminMemberVo}" />


						<!-- 로그인이 되어 있는 경우-->

						<c:if
							test="${loginedAdminMemberVo != null && loginedAdminMemberVo.getAdmin_no() == bulletinVo.getBulletin_writer_no()}">
							...
							<p class="text-right px-2"
								...
								<a href="${delete_url}" class="text-darkgray--">삭제하기</a>
							</p>
						</c:if>
						<!-- 로그인이 되어 안되어있는 경우-->
						<c:if
							test="${loginedAdminMemberVo == null or loginedAdminMemberVo.getAdmin_no() != bulletinVo.getBulletin_writer_no()}">

							<!-- session에 담긴 loginedAdminMemberVo의 값이 null이면 아래 표시 -->
							<p class="text-right px-2"
								style="text-align: right; margin-bottom: 0;">신고하기 | 스크랩</p>
						</c:if>
					</div>
				</div>
			</div>
